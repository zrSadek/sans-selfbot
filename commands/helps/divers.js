...
module.exports = {
    name: "divers",
    descriptionfr: "Affiche divers commandes",
    descriptionen: "Displays various commands",
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/divers`).filter(file => file.endsWith('.js'));
        
        for (const file of commandFiles) {
            const command = require(`../divers/${file}`);
        ...
        client.db.theme === "ryz" ? message.edit(`# divers\n## ${client.trad(`divers commands`, `Commandes divers`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`divers\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
