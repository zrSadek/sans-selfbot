...
module.exports = {
    name: "settings",
    descriptionfr: "Les commandes permettant de modifier les paramètres du bot",
    descriptionen: "Commands to change bot settings",
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/settings`).filter(file => file.endsWith('.js'));
        ...
        client.db.theme === "ryz" ? message.edit(`# settings\n## ${client.trad(`settings commands`, `Commandes settings`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`settings\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
