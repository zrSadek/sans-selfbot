...
module.exports = {
    name: "gestion",
    descriptionfr: "Les commandes pour gérer un serveur",
    descriptionen: "Commands to manage a server",
    premium: true,
    invisible: true,
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/gestion`).filter(file => file.endsWith('.js'));
        ...
        client.db.theme === "ryz" ? message.edit(`# gestion\n## ${client.trad(`gestion commands`, `Commandes gestion`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`gestion\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
