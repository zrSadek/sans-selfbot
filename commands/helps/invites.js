...
module.exports = {
    name: "invites",
    descriptionfr: "Les commandes pour faire des invitations d'amis",
    descriptionen: "Commands to make friends invitations",
    premium: true,
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/invites`).filter(file => file.endsWith('.js'));
        ...
        client.db.theme === "ryz" ? message.edit(`# invites\n## ${client.trad(`invites commands`, `Commandes invites`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`invites\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
