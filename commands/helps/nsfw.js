...
module.exports = {
    name: "nsfw",
    descriptionfr: "Quelques commandes No Safe For Workf",
    descriptionen: "Quelques commandes No Safe For Workf",
    premium: true,
    nsfw: true,
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/nsfw`).filter(file => file.endsWith('.js'));
        ...
        client.db.theme === "ryz" ? message.edit(`# nsfw\n## ${client.trad(`nsfw commands`, `Commandes nsfw`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`nsfw\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
