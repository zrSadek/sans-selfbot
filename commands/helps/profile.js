...
module.exports = {
    name: "profile",
    descriptionfr: "Des commandes pour faciliter l'édition du profile",
    descriptionen: "Controls for easy editing of the profile",
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/profile`).filter(file => file.endsWith('.js'));
        
        for (const file of commandFiles) {
            const command = require(`../profile/${file}`);
        ...
        client.db.theme === "ryz" ? message.edit(`# profile\n## ${client.trad(`profile commands`, `Commandes profile`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`profile\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
