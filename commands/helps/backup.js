...
module.exports = {
    name: "backup",
    descriptionfr: "Liste des commandes pour sauvegarder un serveur",
    descriptionen: "List of commands to backup a server",
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/backup`).filter(file => file.endsWith('.js'));
        
        for (const file of commandFiles) {
            const command = require(`../backup/${file}`);
        ...
        client.db.theme === "ryz" ? message.edit(`# backup\n## ${client.trad(`backup commands`, `Commandes backup`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`backup\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
