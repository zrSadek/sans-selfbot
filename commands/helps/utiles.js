...
module.exports = {
    name: "utiles",
    descriptionfr: "Des commandes utiles ajoutés au selfbot",
    descriptionen: "Useful commands added to the selfbot",
    run: async (client, message, args) => {
        let cc = 0
        const commands = []
        const commandFiles = fs.readdirSync(`./commands/utiles`).filter(file => file.endsWith('.js'));
        
        for (const file of commandFiles) {
            const command = require(`../utiles/${file}`);
        ...
        client.db.theme === "ryz" ? message.edit(`# utiles\n## ${client.trad(`utiles commands`, `Commandes utiles`)}\n${commands.map(c => c).join('\n')}`) :
        client.db.theme === "ethone" ? message.edit(`\`\`\`${client.db.name}\`\`\`\`\`\`utiles\`\`\`\`\`\`${commands.map(c => c).join('\n')}\`\`\``) :
        ""
